...
class SCAdaCos(nn.Module):
    def __init__(self, n_classes=10, n_subclusters=1, out_dims=256, scale=10, shift=8, learnable_loss=False, adaptive_scale=False, warp=True, **kwargs):
        ...
        self.n_subclusters = 1#n_subclusters
    ...
    def forward(self, x, y1):
        ...
        if self.Adaptive_s:
            ...
            """
            max_s_logits = torch.max(self.s * logits.detach())
            B_avg = torch.exp(self.s*logits.detach() - max_s_logits)
            B_avg = torch.sum(B_avg, dim=1).mean(dim=0)
            
            theta_subclass = torch.min(theta.detach().view(-1, self.n_classes, self.n_subclusters), dim=2)[0]
            theta_class = torch.sum(y1_orig * theta_subclass, dim=1) * torch.sum((y1_orig != 0).float(), dim=1)# take mix-upped angle of mix-upped classes)
            theta_med = torch.median(theta_class) # computes median
            self.s[:] = (max_s_logits + torch.log(B_avg)) / (torch.cos(torch.minimum((torch.pi / 4)*torch.ones_like(theta_med), theta_med)))# + self.epsilon)
            #"""
        ...
        CE2 = 10*Cross_Entropy(logits.clone().detach())
