...
from dataset.utils import BalancedBatchSampler
...
parser.add_argument('--resume', default='Solace_Iter:3280.pth')
#parser.add_argument('--resume', default='Solace_Iter:3936.pth')
...
class Trainer(object):
    def __init__(self, cfg, ckpt_path=None):
        ...
        #Prepare Dataset
        TrData, EvData, UnkData, TsData, Meta = prepare_data(target_sr=cfg['target_sr'])
        ...
        Train_Data = Data(TrData, Train=True, SMOTE=SMOTE, Target_sr=cfg['target_sr'], smotecfg=cfg['smotecfg'])
        TrSampler = BalancedBatchSampler(TrData['Labels'], self.n_batch_classes, self.n_class_samples, self.batch_size) if cfg['class_balanced_sampling'] else None
        ...
        if TrSampler is not None:
            ...
            self.Tr1Loader = data.DataLoader(Train_Data, batch_size=self.batch_size, shuffle=True, num_workers=2, pin_memory=True, drop_last=SMOTE) 
            self.Tr2Loader = data.DataLoader(Train_Data, batch_size=self.batch_size, shuffle=True, num_workers=2, pin_memory=True, drop_last=SMOTE)
        ...
        TrEval_Data = Data(TrData, Train=True, SMOTE=False, Target_sr=cfg['target_sr'], smotecfg=cfg['smotecfg'])
        ...
        self.TestLoader = data.DataLoader(Test_Data, batch_size=self.batch_size, num_workers=2, pin_memory=True, drop_last=False)
        ...
        self.model = Wilkinghoff(melcfg=cfg['melcfg'], mdam=cfg['mdam']).to(self.device)
        if cfg['my_loss']:
            ...
            self.Loss = SCAdaCos(n_classes=Meta['num_trclasses'], n_subclusters=self.n_subclusters, out_dims=2*cfg['dimensions'], learnable_loss=cfg['learnable_loss']).to(self.device)
        ...
        self.optimizer = torch.optim.Adam(Parameters, eps=1.0) #Change this...?
        ...
        self.logger = SetupLogger(name="Solace", save_dir=".", distributed_rank=0, filename="flog.txt", mode="a+")
        if self.my_loss:
            self.logger.info(
    ...
    def _checkpoint(self):
        ...
        file = "Solace_Iter:{}.pth".format(self.iteration)
        ...
        file = "Weights_Iter:{}.pth".format(self.iteration)
    ...
    def _eval(self):
        ...
        final_results = evaluate(embeddings, Preds, self.Meta, self.logger, self.evalcfg, n_subclusters=self.n_subclusters)
        self.model.train()  
    ...
    def train(self):
        ...
        for epoch in range(1,self.epochs+1):
            ...
            for i,Samples in enumerate(self.Tr1Loader):#zip(self.Tr1Loader,self.Tr2Loader)):
                self.iteration += 1
